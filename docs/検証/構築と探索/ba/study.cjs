...
const verification = {underfunded_purchase_rejections: 0, refund_replays: 0, refund_undo_checks: 0, refund_save_restores: 0, no_choice_unchanged_saves: 0, economy_conservation_checks: 0, prepared_save_restores: 0, purchase_save_restores: 0, prefix_runs: 0, prefix_preparation_matches: 0, prefix_terminal_matches: 0, checkpoint_conditions: 0,
...
function runBranch(saved, seeds, label, policy) {
  ...
  const decisionSave = s.save(), choiceInput = copy(s.data.economy), operation = 'BA-purchase-' + policy;
  ...
  const g = s.game, receipt = copy(s.data.receipts.at(-1));
...
for (const checkpoint of cfg.reconstruction.checkpoints) {
  ...
  assert.throws(() => M.purchase(saved.economy, offered.id, offered.candidates[0].id, 'BA-underfunded'), /Insufficient unspent/);
  assert.equal(sha(saved.economy), beforeRejected); verification.underfunded_purchase_rejections++; assert(Object.keys(saved.economy.profile.learned).length < 4);
  ...
  histories.push({id: label, ...checkpoint, prefix, checkpoint_sha256: sha(saved), economy_sha256: sha(saved.economy),
    ...
    refund_plan: plan, C_already_cleared: saved.economy.profile.clears.includes('C'), offer: copy(offered), inventory: copy(saved.economy.inventory), next_run: saved.nextRun});
  ...
  process.stdout.write('BA ' + label + ': prefix and two branches complete\n');
}
...
fs.writeFileSync(path.join(__dirname, 'checkpoints-manifest.json'), JSON.stringify({trial: cfg.trial, count: manifests.length,
  full_snapshots_included: false, reconstruction_command: 'node docs/検証/構築と探索/ba/study.cjs', checkpoints: manifests}, null, 2) + '\n');
