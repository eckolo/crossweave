...
版：1.3／2026-09-16（日本時間）

親タスク：**UI-PLAN-001「全画面のUIを決める」**

担当：UI Work `20260910-ui-readability`／`ui/readability-20260910`

状態：**全30項目＋条件待ち4候補の構成と26行動経路を整理し、帰還・札組／心得・探索先・探索をつなぐ操作試作0.9へ更新。文字の大きな囲いを外し、帰還・探索先・場面文を背景へ重ねる配置に修正。16:9の了承と主画面のスクロール禁止を受領し、固定操作帯・枠内ページ一覧・本文の小窓を反映。心得一覧の密度、記録の札詳細と親子窓、探索の収まり・能力アイコン・対象指定と参照の分離を修正。実Campaign＋一時メモリーで主経路を接続。全機能実装・実セーブ・実表示・操作量実測・ユーザー受入とCO-U02全体完了は未了。**
...
**今回の検討入口：[全画面構成0.9](全画面構成.md)・[画面機能要件0.8](画面機能要件.md)・[一巡の操作試作0.9](co-u02/journey/README.md)（2026-09-16）。** 構成資料は全IDと26経路、機能要件は情報・更新境界を保持する。設計を先に見直した後、ユーザーの「この方向で改めてUI設計」を受けて主経路を具体化した。既存一覧・履歴を維持し、機能IDごとに独立画面を作らない。

今回の追加：[探索配置の復旧0.9](co-u02/journey/layout-restore.md)。通常幅で低画面用の配置へ切り替わった回帰を修正し、全幅で相手→場→手札の順と札の高さ上限を維持する。実描画の確認は未了。
...
### 2026-09-16：帰還の区切り・窓内参照・主体値

今回の追加指摘9点を[操作試作0.6](co-u02/journey/flow-review.md)へ反映。帰還は拠点へ送り、編成はその後に選ぶ。現在の編成は対象と数量の行一覧。調査記録は親一覧＋1枚の詳細面を共通規則にし、窓内の下方展開を廃止。場面変更で小窓をリセット、短い追加本文は初期表示。探索は主体4値の常時表示、対象タップ／詳細ホールド、予測の開閉、行動名の短縮を反映した。実Campaign＋MemoryStore／JSDOMの新変更38件・記録31札参照を確認。実描画・実タッチ・IndexedDB・人の受入は未確認。旧正式原本は無変更。

### 2026-09-16：本文の規則・参照入口・変化の予測

[操作試作0.7](co-u02/journey/actor-review.md)へ追加指摘を反映。意味上の段落と自然な行折り返しを分け、文章全体のリンク化をやめて詳細ボタンへ。戻るは左下、現状の参照は構成を見る。探索では隠蔽残量を余力に隣接させ、主体と場への公開予測を選択時に重ねる。用語集1.3に照らして主体の蓄積値は機転へ修正。機転・軽減・攪乱等の解決後予測とruntimeの表示辞書整合は設計への依頼として残す。実Campaign＋MemoryStore／JSDOMの今回変更38件を確認。実描画・IndexedDB・ユーザー受入・正式v0.15完全移植は未了。

### 2026-09-16：操作試作0.8

[句読点・窓・能力表示](co-u02/journey/fit-review.md)を追加。本文の折り返し、直前の窓を残す詳細、UIの所属位置、同じピン、性質欄、隠蔽の再設定前予測を修正。今回44件はJSDOMと実Campaign／一時メモリーによる変更範囲の確認であり、実描画・実セーブ・ユーザー受入を意味しない。固定原本・他担当領域は変更しない。


### 2026-09-16：採用方向のUIと実起動・保存を統合

[操作UI 0.10](co-u02/journey/save-flow.md)。同じ画面から新規作成・続きから・読込み・書出しへ接続し、未完成の変更案も中断・再開で保持。公開行動履歴・獲得記録・札詳細を補完。今回は実Campaign＋MemoryStore／JSDOM49件、実ブラウザーの保存・4幅の描画は確認待ち。[確認手順](co-u02/journey/browser-review.md)。方向性了承を全受入へ広げず、固定原本は維持。
