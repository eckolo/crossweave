...
| W | [世界観設定1.12](https://github.com/eckolo/crossweave/blob/feb8a3f842ebe1e5315cc98a954d1493ebcf9f74/docs/確定仕様/世界観/世界観設定.md)・[正式用語集1.3](https://github.com/eckolo/crossweave/blob/feb8a3f842ebe1e5315cc98a954d1493ebcf9f74/docs/確定仕様/世界観/正式用語集.md) | 古道具屋を拠点にする。心得・余力・緊急脱出などの表現。着想は習得点の仮称候補として扱う |
...
| UI-S03 | 拠点 | 古道具屋から依頼・準備・成長・記録へ進む。帰還後の戻り先 | 場所の見せ方、各機能の入口と近さ、今回できることの示し方 | 未着手／W、S、D 2.18 |
| UI-S04 | 依頼・探索先一覧 | 行ける探索先を比較して詳細・準備へ | 初回・未解決・解決済み・再訪・未解放の区別、候補増加への対応。地理的な地図は前提にしない | 未着手／D 2.20〜2.22、A、S |
| UI-S05 | 依頼・探索先詳細 | 目的、場所の傾向、既知の情報を確認し、準備へ | 初見で見える情報と調査後の情報、解決後の変化。一律の受注・報告操作を追加しない | 試遊案あり／D 6.4、S、T |
| UI-S06 | 出発準備 | 選んだ探索先と今回の編成・装備を確認して出発する | 概要と編集画面の接続、不足・制限違反の示し方。出発前に別の必須確認窓を重ねるかは未決 | 試遊案あり／D 2.6〜2.7、2.18、T |

### 3.2 編成・成長・所持品

| ID | 画面・UI | 役割と出入り | UIで決めること | 状態・根拠 |
|---|---|---|---|---|
| UI-S07 | 札の編成 | 利用可能な札から今回の初期山札を組み、準備へ戻る | 選択元と編成中の札、枚数・制限、追加・除去・詳細比較。現在の探索中の山札とは分ける | 試遊案あり／D 2.6〜2.7、2.18〜2.19、T |
...
| UI-S16 | 帰還結果 | 踏破・撤退・緊急脱出の結果を確認し、拠点や次の準備へ | 同じ画面の3状態として持帰り／喪失、知識、解放、帰還後に使えるものを整理。終了時の余力と帰還による回復を混同しない | 試遊案あり／D 2.13〜2.14、2.18、W、S、T |
...
| 全体構成・画面の分け方 | 未着手・次の作業 | S03〜S06とS16を中心に、拠点→準備→探索→帰還の一巡と共通メニューの案を出す |
| 各画面の配置・操作を決める | 未着手／一部に既存基準 | 各Sタスクの案・比較・採用判断をここへ紐付ける。既存探索UIの了承を再質問しない |
...
今回は文書によるフロー整理とタスク登録。UIコード・ゲーム条件・既存試遊を変更せず、新しい画面の実機操作を確認済みとはしない。次の作業は、全体構成を具体化するための拠点・出発準備・帰還の配置案である。
