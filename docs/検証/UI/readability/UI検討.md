...
版：0.6／2026-09-16（日本時間）

親タスク：**UI-PLAN-001「全画面のUIを決める」**

担当：UI Work `20260910-ui-readability`／`ui/readability-20260910`

状態：**全30項目＋条件待ち4候補の構成を操作量の観点で見直し。主場面4つと起動入口・共通窓の構成案、全26行動経路を整理。心得1画面・札組との往復・全体操作量削減は受領済み要件。具体画面の実装・操作数実測・受入とCO-U02全体完了は未了。**
...
**今回の検討入口：[全画面構成0.1](全画面構成.md)と[画面機能要件0.3](画面機能要件.md)（2026-09-16）。** 構成資料は全IDの配置先・主操作・省く往復・保持する文脈と26行動経路を管理し、機能要件は情報・更新境界を保持する。以下の既存一覧・各試作の履歴は維持し、機能IDごとに独立画面を作らない。ユーザーの指示により、画面制作より先に設計を見直す。
