...
版：0.4／2026-09-16（日本時間）

親タスク：**UI-PLAN-001「全画面のUIを決める」**

担当：UI Work `20260910-ui-readability`／`ui/readability-20260910`

状態：**30項目＋条件待ち4候補を登録済み。今回、「何がどう変わったか」を軸に全項目の機能要件を整理。配置・演出の採用は未了。CO-U02は実Campaignへの一部接続を提出済み、全体完了・実ブラウザー保存・ユーザー受入は未了。**
...
**今回の検討入口：[画面機能要件](画面機能要件.md)（0.1／2026-09-16）。** 各IDの役割・必要情報・操作と更新範囲・出来事に伴う変化の表示・移動先を対応づけた。以下の既存一覧・各試作の履歴は維持し、新しい機能要件案と実装済みの範囲を区別する。ユーザーの指示により、いきなり画面を作らず、この整理を先行する。
