...
版：0.5／2026-09-16（日本時間）

親タスク：**UI-PLAN-001「全画面のUIを決める」**

担当：UI Work `20260910-ui-readability`／`ui/readability-20260910`

状態：**30項目＋条件待ち4候補を登録済み。心得関連をプレイヤー視点で1画面にまとめ、札組との往復と全体の操作量を減らす要件を受領。機能要件0.2に統合構成案・13の行動経路を整理。具体配置・実装・操作数の実測は未了。CO-U02全体完了・実ブラウザー保存・画面のユーザー受入は未了。**
...
**今回の検討入口：[画面機能要件](画面機能要件.md)（0.2／2026-09-16）。** 各IDの役割・必要情報・操作と更新範囲・変化の表示・移動先に加え、札組と心得の共通編集、所持・候補・変換の対象別内包、行動経路全体の操作量を整理。以下の既存一覧・各試作の履歴は維持し、機能IDごとに独立した画面を作らない。ユーザーの指示により、画面制作より先にこの整理を進める。
