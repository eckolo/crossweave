...
function build(replay={build:'guard5',choices:[]}){
  let view=fs.readFileSync(path.join(root,'view.js'),'utf8');
...
if(require.main===module){
  ...
  const out=pos[0]||'/workspace/crossweave-readable.html';
  ...
  let s=build(replay);
  if(standalone)s='<!doctype html>\n<html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>crossweave · UI-R-001</title><style>:root{color-scheme:light dark}body{margin:0 auto;padding:16px;max-width:1056px;font-family:system-ui,sans-serif}</style></head><body>\n'+s+'\n</body></html>\n';
  fs.writeFileSync(out,s);console.log(JSON.stringify({test_id:'UI-R-001',path:out,case:fixture||'start',standalone,bytes:Buffer.byteLength(s),sha256:sha(s)}));
}
