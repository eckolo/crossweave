...
(async()=>{let review;try{
 w.eval(w.document.querySelector('script').textContent);review=w.document.getElementById('cw-u02b-review').__test.review;
 check('配布用と同じ実runtimeの断片が起動する',await review.ready&&root.dataset.screen==='offers');
 ...
 const production=fs.readFileSync('/workspace/crossweave-acquisition-review.html');
 const result={id:'CW-M1-UI-U02B-INLINE-001',node:process.version,jsdom:require(process.env.CW_JSDOM_PATH+'/package.json').version,checks,passed:checks.length,production_sha256:crypto.createHash('sha256').update(production).digest('hex'),production_bytes:production.length,limitation:'JSDOM execution only; no real rendering, touch or IndexedDB'};
 fs.writeFileSync(path.join(__dirname,'inline-checks.json'),JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify({passed:checks.length,errors}));
