...
function build({testing=false,fhd=false,art=false,cards=false,overlap=false,selection=false,placement=false,inward=false}={}){
 art=art||cards||overlap||selection||placement||inward;
 fhd=fhd||art;
...
if(require.main===module){
 const inward=process.argv[3]==='inward',placement=process.argv[3]==='placement',selection=process.argv[3]==='selection',overlap=process.argv[3]==='overlap',cards=process.argv[3]==='cards',art=inward||placement||selection||overlap||cards||process.argv[3]==='art',fhd=art||process.argv[3]==='fhd',result=build({fhd,art,cards,overlap,selection,placement,inward}),output=process.argv[2]||(inward?'/workspace/crossweave-inward-details.html':placement?'/workspace/crossweave-steady-details.html':selection?'/workspace/crossweave-select-details.html':overlap?'/workspace/crossweave-art-overlap.html':cards?'/workspace/crossweave-card-frames.html':art?'/workspace/crossweave-art-hold.html':fhd?'/workspace/crossweave-exploration-full-hd.html':'/workspace/crossweave-ui-consistency.html');
