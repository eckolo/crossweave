...
function build({testing=false}={}){
 ...
 const host=document.getElementById('cw-u02b-review');
 const review=mountReview(host,{ui:CrossweaveUI,cases:checkpoints.filter(c=>Object.hasOwn(packed,c.fixture)),initial:'offers',updateURL:false,prepare:id=>createCheckpoint(id,loadDocument)});`,
...
if(require.main===module){const result=build(),output=process.argv[2]||'/workspace/crossweave-acquisition-review.html';fs.writeFileSync(output,result.html);fs.writeFileSync(path.join(__dirname,'inline-manifest.json'),JSON.stringify({version:'0.12.0',path:output,bytes:Buffer.byteLength(result.html),sha256:sha(result.html),runtime_sources:result.sources,fixtures:result.fixtures},null,2)+'\n');console.log(JSON.stringify({path:output,bytes:Buffer.byteLength(result.html),sha256:sha(result.html)}));}
