...
function build({testing=false,fhd=false,art=false,cards=false,overlap=false,selection=false}={}){
 art=art||cards||overlap||selection;
 fhd=fhd||art;
...
if(require.main===module){
 const selection=process.argv[3]==='selection',overlap=process.argv[3]==='overlap',cards=process.argv[3]==='cards',art=selection||overlap||cards||process.argv[3]==='art',fhd=art||process.argv[3]==='fhd',result=build({fhd,art,cards,overlap,selection}),output=process.argv[2]||(selection?'/workspace/crossweave-select-details.html':overlap?'/workspace/crossweave-art-overlap.html':cards?'/workspace/crossweave-card-frames.html':art?'/workspace/crossweave-art-hold.html':fhd?'/workspace/crossweave-exploration-full-hd.html':'/workspace/crossweave-ui-consistency.html');
