...
function build({testing=false,fhd=false,art=false,cards=false,overlap=false,selection=false,placement=false,inward=false,edge=false,ghost=false}={}){
 art=art||cards||overlap||selection||placement||inward||edge||ghost;
 fhd=fhd||art;
...
if(require.main===module){
 const ghost=process.argv[3]==='ghost',edge=process.argv[3]==='edge',inward=process.argv[3]==='inward',placement=process.argv[3]==='placement',selection=process.argv[3]==='selection',overlap=process.argv[3]==='overlap',cards=process.argv[3]==='cards',art=ghost||edge||inward||placement||selection||overlap||cards||process.argv[3]==='art',fhd=art||process.argv[3]==='fhd',result=build({fhd,art,cards,overlap,selection,placement,inward,edge,ghost}),output=process.argv[2]||(ghost?'/workspace/crossweave-field-forecast-details.html':edge?'/workspace/crossweave-edge-details.html':inward?'/workspace/crossweave-inward-details.html':placement?'/workspace/crossweave-steady-details.html':selection?'/workspace/crossweave-select-details.html':overlap?'/workspace/crossweave-art-overlap.html':cards?'/workspace/crossweave-card-frames.html':art?'/workspace/crossweave-art-hold.html':fhd?'/workspace/crossweave-exploration-full-hd.html':'/workspace/crossweave-ui-consistency.html');
