...
function build({testing=false,fhd=false,art=false,cards=false,overlap=false,selection=false,placement=false,inward=false,edge=false,ghost=false,destinations=false,unified=false}={}){
 ...
 fhd?`const sceneLabels={'explore-d03':'探索','entry-d03':'出発時の本文',carried:'帰還','hub-d03':'${destinations?'探索先選択':'出発前'}'};const cases=checkpoints.filter(c=>sceneLabels[c.id]).map(c=>({...c,label:sceneLabels[c.id]}));`:`const cases=checkpoints.filter(c=>Object.hasOwn(packed,c.fixture));`,
 `const review=mountReview(host,{ui:CrossweaveUI,cases,initial:'${unified?'carried':destinations?'hub-d03':fhd?'explore-d03':'skills-current'}',updateURL:false,prepare:${unified?'async id=>({...await createCheckpoint(id,loadDocument),destinationPreview,acquisitionPreview})':destinations?'async id=>({...await createCheckpoint(id,loadDocument),destinationPreview})':'id=>createCheckpoint(id,loadDocument)'}});`,
