...
function build({testing=false,fhd=false,art=false,cards=false,overlap=false,selection=false,placement=false,inward=false,edge=false,ghost=false,destinations=false}={}){
 ...
 `const review=mountReview(host,{ui:CrossweaveUI,cases,initial:'${destinations?'hub-d03':fhd?'explore-d03':'skills-current'}',updateURL:false,prepare:${destinations?'async id=>({...await createCheckpoint(id,loadDocument),destinationPreview})':'id=>createCheckpoint(id,loadDocument)'}});`,
...
if(require.main===module){
 const destinations=process.argv[3]==='destinations',ghost=process.argv[3]==='ghost',edge=process.argv[3]==='edge',inward=process.argv[3]==='inward',placement=process.argv[3]==='placement',selection=process.argv[3]==='selection',overlap=process.argv[3]==='overlap',cards=process.argv[3]==='cards',art=destinations||ghost||edge||inward||placement||selection||overlap||cards||process.argv[3]==='art',fhd=art||process.argv[3]==='fhd',result=build({fhd,art,cards,overlap,selection,placement,inward,edge,ghost,destinations}),output=process.argv[2]||(destinations?'/workspace/crossweave-destination-selection.html':ghost?'/workspace/crossweave-field-forecast-details.html':edge?'/workspace/crossweave-edge-details.html':inward?'/workspace/crossweave-inward-details.html':placement?'/workspace/crossweave-steady-details.html':selection?'/workspace/crossweave-select-details.html':overlap?'/workspace/crossweave-art-overlap.html':cards?'/workspace/crossweave-card-frames.html':art?'/workspace/crossweave-art-hold.html':fhd?'/workspace/crossweave-exploration-full-hd.html':'/workspace/crossweave-ui-consistency.html');
 fs.writeFileSync(output,result.html);
 const files=['../dist/crossweave-ui.js','../dist/crossweave-ui.css','../display-frame.js','../window-placement.js','../prose-layout.js','../exploration.js','../journey/view.js','../journey/panels.js','../journey/destinations.js','../journey/destinations.css','../journey/layout.js','../journey/launcher.js','../journey/full-hd.css','../journey/build.cjs','../build.cjs','build-inline.cjs','picker.mjs','checkpoints.mjs',fhd?'exploration-fhd.fragment.html':'preview.fragment.html',...(fhd?['display-controls.js']:[]),...(destinations?['destinations.json']:[]),...(art?['../hold-cue.js','../hold-cue.css','../art-assets/build.cjs','../art-assets/presentation.js','../art-assets/presentation.css']:[])];
 fs.writeFileSync(path.join(__dirname,destinations?'destination-selection-manifest.json':ghost?'field-forecast-details-manifest.json':edge?'edge-details-manifest.json':inward?'inward-details-manifest.json':placement?'steady-details-manifest.json':selection?'select-details-manifest.json':overlap?'art-overlap-manifest.json':cards?'card-frames-manifest.json':art?'art-hold-manifest.json':fhd?'exploration-fhd-manifest.json':'inline-manifest.json'),JSON.stringify({version:destinations?'0.14.9':ghost?'0.14.8':edge?'0.14.7':inward?'0.14.6':placement?'0.14.5':selection?'0.14.4':overlap?'0.14.3':cards?'0.14.2':art?'0.14.1':fhd?'0.14.0':'0.13.0',path:output,bytes:Buffer.byteLength(result.html),sha256:sha(result.html),...(art?{script_encoding:'gzip-base64',decoded_script_sha256:result.script_sha256}:{}),runtime_sources:result.sources,fixtures:result.fixtures,ui_sources:files.map(p=>({path:p,sha256:sha(read(p))}))},null,2)+'\n');
