...
function build({testing=false,fhd=false,art=false,cards=false,overlap=false,selection=false,placement=false}={}){
 art=art||cards||overlap||selection||placement;
 fhd=fhd||art;
...
if(require.main===module){
 const placement=process.argv[3]==='placement',selection=process.argv[3]==='selection',overlap=process.argv[3]==='overlap',cards=process.argv[3]==='cards',art=placement||selection||overlap||cards||process.argv[3]==='art',fhd=art||process.argv[3]==='fhd',result=build({fhd,art,cards,overlap,selection,placement}),output=process.argv[2]||(placement?'/workspace/crossweave-steady-details.html':selection?'/workspace/crossweave-select-details.html':overlap?'/workspace/crossweave-art-overlap.html':cards?'/workspace/crossweave-card-frames.html':art?'/workspace/crossweave-art-hold.html':fhd?'/workspace/crossweave-exploration-full-hd.html':'/workspace/crossweave-ui-consistency.html');
