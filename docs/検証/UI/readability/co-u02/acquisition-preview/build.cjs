...
async function makeFixture(){
 ...
 return {version:'two-stage-ui-1',source_design:'7565afca98684186705a4e865d4fdf0e0ae489c1',
...
async function build({write=false,testing=false,output='/workspace/crossweave-two-stage-ui.html'}={}){
 const fixture=await makeFixture();
 const app=read('app.js')+(testing?'\nroot.__test={snapshot:()=>clone({current,draft,view}),fixture};':'');
 new Function('fixture',app);
 const html=read('preview.fragment.html').replace('__STYLE__',()=>read('screen.css')).replace('__FIXTURE__',()=>JSON.stringify(fixture).replace(/</g,'\\u003c')).replace('__APP__',()=>app.replace(/<\/script/gi,'<\\/script'));
 ...
 if(write){
  ...
  fs.writeFileSync(path.join(__dirname,'inline-manifest.json'),JSON.stringify({version:fixture.version,path:output,bytes:Buffer.byteLength(html),sha256:hash(html),source_design:fixture.source_design,scope:'UI操作確認用。共通Campaign・本編保存・戦闘試用は未接続。',sources:['app.js','screen.css','preview.fragment.html','build.cjs'].map(p=>({path:p,sha256:hash(read(p))}))},null,2)+'\n');
 }
 return {html,fixture};
}
if(require.main===module)build({write:true,output:process.argv[2]||'/workspace/crossweave-two-stage-ui.html'}).then(({html})=>console.log(JSON.stringify({bytes:Buffer.byteLength(html),sha256:hash(html)}))).catch(e=>{console.error(e);process.exitCode=1;});
