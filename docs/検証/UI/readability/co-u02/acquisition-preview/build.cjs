...
async function makeFixture(){
 ...
 const offers=[
  ...
  {id:'offer-long',key:item('passive','PS01',['borrowed','focused_B','sluggish']),price:4},
  {id:'offer-tide',key:item('card','nt_stop',['light']),price:4},
  {id:'offer-force',key:item('card','f',['heavy']),price:8}
 ];
 return {version:'two-stage-ui-5.1',source_design:'7565afca98684186705a4e865d4fdf0e0ae489c1',
...
async function build({write=false,testing=false,output='/workspace/crossweave-owned-labels.html'}={}){
 const fixture=await makeFixture();
 const app=read('app.js').replace('__LAYOUT__',()=>read('layout.js')).replace('__PREVIEW__',()=>read('preview.js')).replace('__GESTURES__',()=>read('gestures.js'))+(testing?'\nroot.__test={snapshot:()=>clone({current,draft,view}),fixture,layout:dimensions,preview:()=>clone(previewState),resizePreview:syncPreview};':'');
 ...
 if(write){
  ...
  fs.writeFileSync(path.join(__dirname,'inline-manifest.json'),JSON.stringify({version:fixture.version,path:output,bytes:Buffer.byteLength(html),sha256:hash(html),source_design:fixture.source_design,scope:'UI操作確認用。共通Campaign・本編保存・戦闘試用は未接続。',sources:['app.js','layout.js','preview.js','gestures.js','structure.css','preview.fragment.html','build.cjs'].map(p=>({path:p,sha256:hash(read(p))}))},null,2)+'\n');
 }
 return {html,fixture};
}
if(require.main===module)build({write:true,output:process.argv[2]||'/workspace/crossweave-owned-labels.html'}).then(({html})=>console.log(JSON.stringify({bytes:Buffer.byteLength(html),sha256:hash(html)}))).catch(e=>{console.error(e);process.exitCode=1;});
