...
async function makeFixture(){
 ...
 return {version:'two-stage-ui-5',source_design:'7565afca98684186705a4e865d4fdf0e0ae489c1',
...
async function build({write=false,testing=false,output='/workspace/crossweave-full-hd.html'}={}){
...
if(require.main===module)build({write:true,output:process.argv[2]||'/workspace/crossweave-full-hd.html'}).then(({html})=>console.log(JSON.stringify({bytes:Buffer.byteLength(html),sha256:hash(html)}))).catch(e=>{console.error(e);process.exitCode=1;});
