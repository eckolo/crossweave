...
UI0.14.3で潜水服の表示を高さ176px、枠上端−8px、下側はステータスの後ろへ変更。相手枠320×248px、画像ファイルと原本は不変。[配置条件](../journey/art-overlap.md)。札の実画像もはみ出しと下側の重なりを許容し、導入時には共通札面とスクロール余白へ接続する。現在の札は記号表示であり、実画像の接続・検証は未実施。

2026-09-25の追加指摘：潜水服の姿勢は了承、描き方は背景と不一致。今回は画像を維持し、新規制作・差替は[ビジュアル担当の正式手順と作例](../journey/card-frames.md)に従う。UI0.14.2で水路画像を地形の対象枠・行動順へ使う処理は除去した。全面背景での使用は保持。以下は素材作成時の記録。
