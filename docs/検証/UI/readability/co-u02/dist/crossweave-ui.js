...
(function(api){'use strict';
 ...
 const menuButton=(kind,name,shape)=>`<button type="button" data-x="${kind}" aria-label="${name}" data-tooltip="${name}">${symbol(shape,({flag:'⚑',activity:'◎','list-ordered':'≡',layers:'▤',history:'↶','sliders-horizontal':'⚙','message-square':'…','book-open':'▣',menu:'☰'})[shape])}<span>${name}</span></button>`;
 ...
 api.mountExploration=function(root,{session,display_data,onScene,onWithdraw,onCommon}){
  ...
  const stat=(key,value)=>{const def={power:['arrow-up-right','↗','突破'],hit:['scan-search','⌖','探査'],guard:['shield','◇','身構'],evasion:['wind','≋','攪乱'],heal:['heart-plus','+','回復'],hp:['heart','♡','余力']}[key];return `<span class="cw-stat" data-stat="${key}" role="img" aria-label="${esc(label(key,def[2]))} ${esc(value)}" data-tooltip="${esc(label(key,def[2]))}">${symbol(def[0],def[1])}<b>${esc(value)}</b></span>`;};
  ...
  function redraw(){if(dead||!x())return;const c=card();
   ...
   $('#cw-self').innerHTML=stat('hp',x().self.hp+'/'+x().self.max_hp);$('#cw-hand-count').textContent=`手札 ${hand().length}枚`;
  ...
  root.addEventListener('click',async e=>{
   ...
   const info=e.target.closest('[data-x-actor-detail]');if(info){open('actor',info.dataset.xActorDetail);return;}
