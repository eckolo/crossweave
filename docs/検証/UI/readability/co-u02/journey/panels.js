...
function returnView(){const r=d().return_receipt,labels={clear:'踏破',withdrawal:'撤退',defeat:'緊急脱出'};
 ...
 return '<section class="cj-result"><div class="cj-landscape" aria-hidden="true"><div></div><div></div><div></div></div><div class="cj-result-top"><span class="cj-eyebrow">帰還</span><h1>'+esc(labels[r.outcome]||'探索終了')+'</h1><p>'+esc(title)+'</p></div>'+sceneCopy()+
 ...
 '<div class="cj-result-actions">'+button('同じ場所へ出発 '+icon('arrow-right'),'depart','data-j-mutation','cj-primary')+'<div>'+button('札組','deck','data-j-mutation')+button('心得','skills','data-j-mutation')+button('探索先','hub','data-j-mutation')+'</div></div></section>';
}
...
function skillTile(id){const item=info(id),on=equipped(id),known=learned(item.base_id),cancelled=p().cancel_learning.includes(item.base_id),newly=p().next_preparation.learn.includes(item.base_id);
 ...
 return '<article class="cj-skillpiece '+(on?'cj-included':'')+'" data-piece="'+esc(id)+'"><div class="cj-skill-heading">'+button(mark(id)+'<strong>'+esc(name(id))+'</strong>','detail','data-id="'+esc(id)+'" data-focus="detail-'+esc(id)+'" aria-pressed="'+(selected.skills===id)+'"','cj-object')+'<span class="cj-slot-cost">枠 '+item.equipment_cost+'</span></div><p>'+esc(item.effect_text||'効果の詳細は未提供')+'</p><small>'+esc(item.trigger_text||'')+'</small><div class="cj-skill-action"><span class="cj-state-label">'+(on?'✓ 装備':label)+'</span>'+button(on?'外す':known?'装備':cancelled?'覚えた状態に戻す':'覚えて装備 −'+pt(item.learning_cost_units),cancelled?'restore-skill':'equip','data-id="'+esc(id)+'" data-focus="equip-'+esc(id)+'" data-j-mutation',on?'':'cj-primary')+'</div></article>';
}
...
function detailView(id){const item=info(id),base=item.base_id,passive=item.kind==='passive',known=learned(base),cancelled=p()?.cancel_learning.includes(base);
 ...
 let body='<section class="cj-inspect-item"><div class="cj-inspect-top"><h2>'+esc(name(id))+'</h2>'+button(icon('pin'),'pin','data-id="'+esc(id)+'" aria-label="固定" aria-pressed="'+!!w?.pinned+'"','cj-icon-button')+button(icon('x'),'close-item','data-id="'+esc(id)+'" aria-label="詳細を閉じる"','cj-icon-button')+'</div>'+mark(id);
 if(item.trigger_text)body+='<p>'+esc(item.trigger_text)+'</p>';if(item.effect_text)body+='<p>'+esc(item.effect_text)+'</p>';
 ...
 if(d().phase==='home'&&passive){body+='<div class="cj-detail-actions">'+button(equipped(id)?'装備から外す':known?'装備':'覚えて装備 −'+pt(item.learning_cost_units),'equip','data-id="'+esc(id)+'" data-j-mutation '+(cancelled?'disabled':''),'cj-primary');
  if(known)body+=button('忘れる','forget','data-id="'+esc(id)+'" data-j-mutation');
  else if(cancelled)body+=button('覚えた状態に戻す','restore-skill','data-id="'+esc(id)+'" data-j-mutation');
  else body+=button('覚える −'+pt(item.learning_cost_units),'learn','data-id="'+esc(id)+'" data-j-mutation');
  body+='</div>'+(cancelled?'<p>この心得の装備が外れる</p>':'');
 }else if(d().phase==='home'&&h().free_card_options.includes(id))body+='<div class="cj-detail-actions">'+button('−1枚','remove','data-id="'+esc(id)+'" data-j-mutation '+(!count(id)?'disabled':''))+ '<strong>'+count(id)+'枚</strong>'+button('+1枚','add','data-id="'+esc(id)+'" data-j-mutation '+(p().next_preparation.deck.filter(x=>info(x).base_id===base).length>=h().deck.per_base_cap?'disabled':''))+'</div>';
 else if(!(h()?.free_card_options||[]).includes(id)&&item.kind==='card')body+='<p class="cj-muted">記録に追加。札組への追加はまだ未対応。</p>';
 return body+'</section>';
}
function renderPanel(){const box=$('[data-inspector]');box.hidden=!panel;$('[data-inspector]').parentElement.classList.toggle('cj-has-inspector',!!panel);if(!panel){box.replaceChildren();return;}
 ...
 let body='';
 if(panel==='review')body=wallet()+changeRows()+'<p class="cj-muted">所持品の増減なし</p><div class="cj-detail-actions">'+button('確定','commit','data-j-mutation '+(!state.comparison?.ok?'disabled':''),'cj-primary')+button('確定して出発','depart','data-j-mutation '+(!state.comparison?.ok?'disabled':''))+'</div>';
 if(panel==='records'){body=list(d().knowledge_views).map(x=>'<section><h3>'+esc(x.name)+'</h3><p>'+ (x.encountered?'遭遇済み':'')+'</p></section>').join('');const known=(d().case?.unlocked_card_ids||[]).map(x=>'base:'+x).filter(id=>info(id).name);body+='<h3>札</h3><div class="cj-record-list">'+known.map(id=>detailsButton(id)).join('')+'</div>';}
 ...
 box.innerHTML='<section class="cj-inspect-item"><div class="cj-inspect-top"><h2>'+titles[panel]+'</h2>'+button(icon('x'),'close','','cj-icon-button')+'</div>'+body+'</section>';
}
