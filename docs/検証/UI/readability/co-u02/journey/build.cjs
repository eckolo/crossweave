...
function build({testing=false}={}){
 ...
 const script=[runtime.code,read('../session.js'),read('../window-placement.js'),read('../action-forecast.js'),read('../exploration.js'),read('layout.js'),view,
...
if(require.main===module){const output=process.argv[2]||'/workspace/crossweave-action-forecast.html';const {html}=build();fs.writeFileSync(output,html);console.log(JSON.stringify({path:output,bytes:Buffer.byteLength(html),sha256:crypto.createHash('sha256').update(html).digest('hex')}));}
