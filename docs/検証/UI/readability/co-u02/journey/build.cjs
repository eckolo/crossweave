...
if(require.main===module){const output=process.argv[2]||'/workspace/crossweave-clear-flow.html';const {html}=build();fs.writeFileSync(output,html);console.log(JSON.stringify({path:output,bytes:Buffer.byteLength(html),sha256:crypto.createHash('sha256').update(html).digest('hex')}));}
