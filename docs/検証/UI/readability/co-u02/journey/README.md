...
| 中断・再開 | 探索中にメニュー→探索を中断、続きから→同じ探索 | 編成は確定時に自動保存。通常入口は実IndexedDBへ接続、会話内のみ一時メモリー |
...
| 機能ID | この試作での扱い |
|---|---|
| S01・S02・S25・S26 | 新規作成・再開・空枠への読込み・書出しを同じUIへ統合。中断は探索のみ、編成は確定時保存。旧版の保存済み案の読込み互換を維持 |
