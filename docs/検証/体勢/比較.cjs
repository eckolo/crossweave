'use strict';
// AL1: execute the frozen real resolver and an isolated candidate resolver.
const fs = require('fs'), path = require('path'), vm = require('vm');
const assert = require('assert/strict'), crypto = require('crypto');
const root = path.resolve(__dirname, '../../..');
const cfg = JSON.parse(fs.readFileSync(path.join(__dirname, '入力.json'), 'utf8'));
const original = fs.readFileSync(path.join(root, cfg.engine), 'utf8');
const hash = s => crypto.createHash('sha256').update(s).digest('hex');
assert.equal(hash(original), cfg.engine_sha256);
const copy = x => JSON.parse(JSON.stringify(x));
let candidate = original;
const patches = [
  ['if(d.hit>=100){', 'if(d.hit>=(d.max_posture??100)){', 1],
  ['hm=Math.floor((100+d.hit)/100)', 'hm=1+Math.floor((d.hit-(d.max_posture??100))/100)', 1],
  ['total>=100', 'total>=(d.max_posture??100)', 2],
  ['*(1+Math.floor(total/100))', '*(1+Math.floor((total-(d.max_posture??100))/100))', 1],
  ['a.hit<100', 'Number.isInteger(a.max_posture??100)&&(a.max_posture??100)>=1&&a.hit<(a.max_posture??100)', 1],
];
for (const [before, after, count] of patches) {
  assert.equal(candidate.split(before).length - 1, count, 'source location changed: ' + before);
  candidate = candidate.split(before).join(after);
}
function load(source) {
  const ctx = {module: {exports: {}}, exports: {}};
  vm.runInNewContext(source, ctx, {filename: 'AL1-isolated-engine.js'});
  return ctx.module.exports.Game;
}
const Old = load(original), Proposed = load(candidate);
...
fs.writeFileSync(path.join(__dirname,'結果.json'),JSON.stringify({trial:cfg.trial,
  base_commit:cfg.base_commit,engine_sha256:hash(original),candidate_sha256:hash(candidate),
  input_sha256:hash(fs.readFileSync(path.join(__dirname,'入力.json'))),
  comparison_sha256:hash(fs.readFileSync(__filename)),patches,rows,reference,sequences,checks,
  scope:'実エンジンの解決・予測と独立派生を人工局面で比較。新しい通し探索・バランス・人評価ではない'},null,2)+'\n');
console.log(JSON.stringify({threshold_cases:rows.length,current_comparisons:reference.length,
  sequences:sequences.length,boundary_checks:checks.length,current_first_hit_damage:reference.find(r=>r.gain===100)}));
