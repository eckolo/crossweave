...
function card(g,b,{user='P',origin=user,attr=null,match=true,kind=null}={}){
  g.bundle.ao.cards[b.key]=copy(b);
  ...
  if(attr)assert.equal(c.attr,attr,'Fixture must preserve authored attribute');if(kind)assert.equal(c.kind,kind);if(c.evasion===undefined)c.evasion=0;c.remaining=c.life;
...
for(const b of variants.passive)for(const origin of cfg.contexts.origins)for(const attr of (b.base==='PS04'?[A.catalog.salve.attr]:cfg.contexts.attributes)){
  const base=b.base,selected={[base]:b},g=fixture(100,40,[base],selected);
  const cb=A.blueprint('card',base==='PS03'?(attr==='B'?'brace':'read'):base==='PS04'?'salve':(attr==='B'?'l':'j'));
...
assert.throws(()=>A.blueprint('passive','PS04',['focused_B']),/Incompatible trigger attribute/);
