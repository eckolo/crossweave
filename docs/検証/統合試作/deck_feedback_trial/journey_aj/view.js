  ...
  function render(){
    ...
    if(c&&!s.outcome&&!needsTarget){
      ...
      let brief=pred.mode==='place'?`場への補正：攻撃／防御 ${signed(c.field_power)}、命中／回避 ${signed(c.field_hit)}。コスト ${pred.action_cost}`:pred.mode==='attack'?`${names[target]}：${pred.hit_connected?'命中':'未命中'}、HP ${s.actors[target].hp} → ${s.actors[target].hp-pred.actual_hp_loss}`:pred.mode==='guard'?`防御 ${pred.guard.value}、回避 ${signed(pred.guard.evasion)}、命中する攻撃2回まで`:pred.mode==='heal'?`HP ${p.hp} → ${p.hp+pred.hp_restored}`:'一致 · 会心増加';
  ...
  for(const id of ['cw-quick-setting','cw-drag-setting','cw-button-setting'])get(id).addEventListener('change',()=>{cancelDrag();persist();render();});
