/* AJ1: human-controlled preparation, expedition, settlement and retry. */
'use strict';
const AH=require('../expedition_choices.js'),AI=require('../reward_preparation.js');
const L=require('../expedition_loop.js'),K=require('../knowledge.js');
const copy=AH.copy;
const check=(v,m)=>{if(!v)throw Error(m);};
const skillText={
  PS01:{name:'守りから展開',text:'防御の一致直後、次の自分の行動が設置なら行動コスト−2（最低1）。別の行動では失効。'},
  PS02:{name:'属性をつなぐ',text:'直前の自分の一致と異なる属性で攻撃を一致させると、その攻撃の命中加算＋20。'},
  PS03:{name:'借り札を守りへ',text:'他者由来の札を一致させると、次の防御一致の防御値＋2。重複せず、発動後に再び条件を満たせる。'},
  PS04:{name:'回復を伸ばす',text:'消費型の回復札を一致させると、回復量＋4。'}
};
const purposeText={passage:'通路',terminal:'最終環境',optional_enemy:'任意の敵',goal_enemy:'目的の敵',support:'支援環境',weak_environment:'弱まった環境'};
class Session{
  constructor(source,seeds,saved=null){
    this.source=source;this.seeds=seeds;
    this.data={schema:'AJ1',phase:'home',profile:AH.initialProfile(),counts:AH.countsFor(source,'guard3'),intent:'',route:'A',nextRun:0,receipt:null};
    this.game=null;this.bundle=null;
    if(saved)this.restore(saved);
  }
  restore(saved){
    check(saved?.schema==='AJ1','この試作の保存データではありません');
  ...
  depart(route,counts,skills,intent=''){
    ...
    const out=AH.depart(this.source,this.seeds[index%this.seeds.length],planned.profile,planned.counts,route,'AJ1-'+index);
