(() => {
  const source=CWRequire('input.json'),seedBank=CWRequire('posture_am/seeds.json');
  const AH=CWRequire('expedition_choices.js'),L=CWRequire('expedition_loop.js'),I=CWRequire('information.js');
  const CWFeedback=CWRequire('feedback.js'),AJ=CWRequire('posture_am/session.js');
  let bundle;
  const root=document.getElementById('cw-playtable');
  const get=id=>root.querySelector('#'+id);
  let names={P:'あなた'},rewardNames={};
  const phases={'A/start':'A · 通路の突破','A/terminal':'A · 霧の突破','A/finished':'A · 完了','B/start':'B · 目的敵の撃破','B/weak':'B · 支援終了','B/finished':'B · 完了','C/start':'C · 通路の突破','C/enemy':'C · 目的敵の撃破','C/finished':'C · 完了'};
  ...
  function logText(r) {
    if(r.type==='action'){
      ...
      else if(r.mode==='attack')result=`${who}：${cardName(c)} → ${names[r.target]}。${r.hit_connected?`体勢を削り切る（${r.posture_multiplier}倍）、HP −${r.actual_hp_loss}`:`体勢 −${r.hit_gain}（残り ${r.posture_after}）`}`;
  ...
  function previewText(c,pred){
    ...
    if(pred.mode==='place')text=`${c.attr}の場へ設置。次に一致した札へ、${fieldText(c).replace('<br>','、')}。`;
      ...
      const d=game.s.actors[target];
      text=`${names[target]}：${pred.hit_connected?'体勢を削り切る':'体勢を削る'}、HP ${d.hp} → ${d.hp-pred.actual_hp_loss}。体勢 ${pred.posture_before} → ${pred.hit_connected?'0、解決後 ':''}${pred.posture_after}/${d.max_posture}。`;
      if(pred.hit_connected)text+=` 超過 ${pred.posture_overflow}、体勢倍率 ${pred.posture_multiplier}倍。`;
      if(pred.hit_gain===0)text+=` 体勢削り0（札 ${c.hit}、場 ${signed(game.s.cards[m].field_hit)}、対象回避 ${game.evasion(target)}）。`;

    } else if(pred.mode==='guard')text=`防御 ${pred.guard.value}・回避 ${signed(pred.guard.evasion)}、体勢を削り切る攻撃へ残2回。`;
  ...
  const mainText=c=>CWFeedback.mainText(c).replace('命中','体勢削り');
  ...
  function prediction(c,pred){
    ...
    if(pred.mode==='place'){
      ...
      const d=game.s.actors[target];heading=`${names[target]}へ攻撃 · ${pred.hit_connected?'体勢を削り切る':'体勢を削る'}`;
      impacts=[`HP ${d.hp} → ${d.hp-pred.actual_hp_loss}`,`体勢 ${pred.posture_before} → ${pred.hit_connected?'0・解決後 ':''}${pred.posture_after}/${d.max_posture}`];
      if(!pred.hit_gain)impacts.push('体勢削り 0');
    }else if(pred.mode==='guard'){
      heading='自分の防御を開始';impacts=[`防御 ${pred.guard.value}`,`回避 ${signed(pred.guard.evasion)}`,`体勢を削り切る攻撃 2回まで`];
