  ...
  function render(){
    ...
    get('cw-self').innerHTML=`<span>あなた　HP ${p.hp}/${p.max_hp}</span><span>体勢 ${p.posture_remaining}/${p.max_posture} · 会心 ${p.crit}</span><span>${p.guard?esc(guardText(p)):'防御なし'}</span>`;
    get('cw-actors').innerHTML=active.map(w=>{const a=s.actors[w];return `<button type="button" class="cw-actor" data-target="${w}" aria-pressed="${target===w}"><span class="cw-actor-head"><strong>${names[w]}</strong><span>HP ${a.hp}/${a.max_hp}</span></span><progress value="${a.hp}" max="${a.max_hp}" aria-label="${names[w]}のHP"></progress><span>体勢 ${a.posture_remaining}/${a.max_posture} · 会心 ${a.crit}${a.guard?' · 防御中':''}</span></button>`;}).join('');
    ...
    get('cw-actor-details').innerHTML=['P',...active].map(w=>{const a=s.actors[w];return `<div class="cw-log">${names[w]}：HP ${a.hp}/${a.max_hp}、体勢 ${a.posture_remaining}/${a.max_posture}、会心 ${a.crit}、${esc(guardText(a))}、回避合計 ${signed(a.evasion)}、軽減 ${a.reduction||0}${a.acts?'、次回 '+a.next_at:''}</div>`;}).join('');
    ...
    const config=settings();root.dataset.cardDrag=String(config.drag);
    get('cw-hand').innerHTML=p.hand.map(x=>{const match=!!s.field[x.attr];return `<article class="cw-hand-card" data-hand-id="${x.id}" data-selected="${selected===x.id}"><button type="button" class="cw-select" data-card="${x.id}" aria-pressed="${selected===x.id}" aria-describedby="cw-gesture-hint" ${s.outcome?'disabled':''}><strong>${attrBadge(x.attr)}${esc(cardName(x))}</strong><span>${mainText(x)}</span><span>由来：${esc(names[x.origin]||x.origin)}${x.origin!=='P'?' · 借り札':''}</span><span class="${x.remaining===1?'cw-loss':''}">${x.remaining===1?'この手まで':'残り'+x.remaining+'手'} · ${match?'一致':'設置'} ${Math.max(1,game.cost(x.type,match)-game.effect('P',x).discount)}</span>${x.consume_on_recover||x.doomed?'<span class="cw-loss">回収で消滅</span>':''}</button></article>`;}).join('');
    ...
    if(c&&!s.outcome&&!needsTarget){
      ...
      let brief=pred.mode==='place'?`場への補正：攻撃／防御 ${signed(c.field_power)}、命中／回避 ${signed(c.field_hit)}。コスト ${pred.action_cost}`:pred.mode==='attack'?`${names[target]}：${pred.hit_connected?'体勢を削り切る':'体勢を削る'}、HP ${s.actors[target].hp} → ${s.actors[target].hp-pred.actual_hp_loss}`:pred.mode==='guard'?`防御 ${pred.guard.value}、回避 ${signed(pred.guard.evasion)}、体勢を削り切る攻撃2回まで`:pred.mode==='heal'?`HP ${p.hp} → ${p.hp+pred.hp_restored}`:'一致 · 会心増加';
      ...
      if(mid)brief+=` · コスト ${pred.action_cost} · 会心 +${pred.crit_added}`;
      ...
      if(pred.passives.length)brief+='。習得：'+pred.passives.map(id=>AJ.skillText[id].name).join('・');
      if(mid&&c.kind==='attack')brief+='。'+consequence(s.actors[target]);
      get('cw-brief').textContent=brief;
    }else{get('cw-brief').textContent=s.outcome?'探索終了。参照から成果を確認できます。':needsTarget?'上の相手・環境から攻撃対象を選んでください。':'札をタップして選択。少し押してつかむと、場へドラッグできます。';get('cw-prediction').textContent=get('cw-brief').textContent;get('cw-prediction-detail').textContent='';}
    ...
    get('cw-start-state').textContent=pendingText(s.passive_state);

    get('cw-withdraw').disabled=!!s.outcome&&s.outcome!=='cutoff';
  ...
  function perform(id,expectedVersion){
    ...
    try{game=session.action({card_id:id,target:c.kind==='attack'&&mid?target:null});version++;recordChanges(before);changedAttrs=new Set(Object.keys(game.public().field).filter(attr=>before.field[attr]?.id!==game.public().field[attr]?.id));absorb();selected=null;notice='';busy=false;persist();renderJourney();return true;}
    catch(error){game=session.game;busy=false;notice='処理を中断しました：'+error.message;render();return false;}
  }
  ...
  for(const id of ['cw-quick-setting','cw-drag-setting','cw-hold-setting'])get(id).addEventListener('change',()=>{cancelDrag();persist();render();});
  get('cw-withdraw').addEventListener('click',()=>{cancelDrag();const before=game.public();game=session.withdraw();version++;recordChanges(before);absorb();selected=null;closePanel();persist();renderJourney();});
