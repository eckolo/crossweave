  const appGet=id=>document.getElementById(id),storageKey='crossweave-AM1';
  ...
  try{
    ...
    if(raw){const saved=JSON.parse(raw);session=new AJ.Session(source,seedBank,saved.session);history=Array.isArray(saved.history)?saved.history.filter(x=>typeof x==='string').slice(-600):[];draft=saved.draft||null;
      for(const [key,id]of Object.entries({quick:'cw-quick-setting',drag:'cw-drag-setting'}))if(typeof saved.settings?.[key]==='boolean')get(id).checked=saved.settings[key];if([150,220,320].includes(saved.settings?.hold))get('cw-hold-setting').value=String(saved.settings.hold);}
  ...
  function snapshot(){return {schema:'AM1-file',session:session.save(),draft,settings:settings(),history:history.slice(-600)};}
  function persist(){
  ...
  function pendingText(p){
    ...
    if(p.learned.includes('PS02'))pending.push(p.last_match_attr?`直前の一致 ${p.last_match_attr}（別属性の攻撃で体勢削り＋20）`:'属性連携：最初の一致待ち');
  ...
  appGet('aj-export').addEventListener('click',()=>{const blob=new Blob([JSON.stringify(snapshot())],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='crossweave-AM1-save.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);});
  appGet('aj-import').addEventListener('change',async event=>{
    const file=event.target.files[0];if(!file)return;
    try{const saved=JSON.parse(await file.text());if(saved.schema!=='AM1-file')throw Error('AM1の保存ファイルを選んでください');const next=new AJ.Session(source,seedBank,saved.session);session=next;draft=saved.draft||null;history=Array.isArray(saved.history)?saved.history.filter(x=>typeof x==='string').slice(-600):[];last=[];selected=null;target=null;knownAttrs=new Set();changes=[];beforeTime=null;version++;get('cw-drawer').hidden=true;loadingError=false;saveError='';
      for(const [key,id]of Object.entries({quick:'cw-quick-setting',drag:'cw-drag-setting'}))if(typeof saved.settings?.[key]==='boolean')get(id).checked=saved.settings[key];if([150,220,320].includes(saved.settings?.hold))get('cw-hold-setting').value=String(saved.settings.hold);persist();renderJourney();
