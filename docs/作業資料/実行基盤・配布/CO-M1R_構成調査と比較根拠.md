...
**位置付け更新（0.2／2026-09-26）**：本書は0.1時点の既存試作の構成調査。コード再利用を重視した旧選定理由は現行推奨ではない。ユーザー追加条件を受けた現行結論は[性能・配布条件での再比較](CO-M1R_性能・配布条件での再比較.md)と[方式案0.2](../../仕様案/カード探索ゲーム_M1実行基盤と配布案.md)。構成・固定結果の原記録は保全する。
