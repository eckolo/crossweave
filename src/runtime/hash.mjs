// Synchronous SHA-256 for the pinned AT weighted generator in both browser and Node.
// UTF-8 input, unsigned big-endian words. Tested against node:crypto, including Unicode.
...
export function sha256(text){
  ...
  const view=new DataView(bytes.buffer),bits=BigInt(input.length)*8n;
  view.setUint32(bytes.length-8,Number(bits>>32n));view.setUint32(bytes.length-4,Number(bits&0xffffffffn));
